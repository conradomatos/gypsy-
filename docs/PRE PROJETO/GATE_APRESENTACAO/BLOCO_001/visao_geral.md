...
**Fontes de dados (Spec 0 — Destilação):** HOLLOS (ex-Pequenas Obras, 35 abas), MURILO (ex-FAZZER, 39 abas), Planilha insumos Concept, Arquimedes, SINAPI-PR, RSMeans Electrical, catálogo Siemens (~100k produtos).

**Base existente (PWC):** 18 telas funcionais, ~9.000 linhas de código, pricebook multi-contexto, WBS hierárquica, sistema de revisões. Gap principal: composições/assemblies, fatores de ajuste, faixa min/prov/máx, IA, motor paramétrico.
